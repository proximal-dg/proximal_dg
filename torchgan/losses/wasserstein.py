...
from .loss import DiscriminatorLoss, GeneratorLoss, ProximalGeneratorLoss, ProximalDiscriminatorLoss

__all__ = [
    "WassersteinGeneratorLoss",
    "WassersteinDiscriminatorLoss",
    "WassersteinGradientPenalty",
    "ProximalWassersteinDiscriminatorLoss",
    "ProximalWassersteinGeneratorLoss"
]
...
class ProximalWassersteinDiscriminatorLoss(ProximalDiscriminatorLoss):
    r"""Wasserstein GAN generator loss from
    `"Wasserstein GAN by Arjovsky et. al." <https://arxiv.org/abs/1701.07875>`_ paper
    ...
    def __init__(self, reduction="mean", clip=None, override_train_ops=None,lamda_prox=0.10,steps=10):
        super(ProximalWassersteinDiscriminatorLoss, self).__init__(
            reduction, override_train_ops,lamda_prox=lamda_prox,steps=steps
        )
        if (isinstance(clip, tuple) or isinstance(clip, list)) and len(clip) > 1:
            self.clip = clip
        else:
    ...
    def train_ops(
        ...
        if self.override_train_ops is not None:
            ...
            if self.clip is not None:
                for p in discriminator.parameters():
                    p.data.clamp_(self.clip[0], self.clip[1])
            return super(ProximalWassersteinDiscriminatorLoss, self).train_ops(
                generator,
                discriminator,
                optimizer_discriminator,
                real_inputs,
...
class ProximalWassersteinGeneratorLoss(ProximalGeneratorLoss):
    r"""Wasserstein GAN generator loss from
    `"Wasserstein GAN by Arjovsky et. al." <https://arxiv.org/abs/1701.07875>`_ paper
