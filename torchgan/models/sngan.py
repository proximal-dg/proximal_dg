...
__all__ = ["SNGANGenerator", "SNGANDiscriminator"]


class SNGANGenerator(Generator):
    r"""Spectral Normalized GAN (SNGAN) generator from
    ...
    Args:
        ...
        step_channels (int, optional): Number of channels in multiples of which the SNGAN steps up
    ...
    def __init__(
        ...
        super(SNGANGenerator, self).__init__(encoding_dims, label_type)
        ...
        if batchnorm is True:
            model.append(
                nn.Sequential(
                    SpectralNorm2d(nn.ConvTranspose2d(self.encoding_dims, d, 4, 1, 0, bias=use_bias)),
                    nn.BatchNorm2d(d),
                    nl,
                )
            )
            for i in range(num_repeats):
                model.append(
                    nn.Sequential(
                        SpectralNorm2d(nn.ConvTranspose2d(d, d // 2, 4, 2, 1, bias=use_bias)),
                        nn.BatchNorm2d(d // 2),
                        nl,
                    )
                )
        ...
        model.append(
            nn.Sequential(SpectralNorm2d(nn.ConvTranspose2d(d, self.ch, 4, 2, 1, bias=True)), last_nl)
        )
...
class SNGANDiscriminator(Discriminator):
    r"""Deep Convolutional GAN (SNGAN) discriminator from
    `"Unsupervised Representation Learning With Deep Convolutional Generative Aversarial Networks
    by Radford et. al. " <https://arxiv.org/abs/1511.06434>`_ paper
    ...
    def __init__(
        ...
        super(SNGANDiscriminator, self).__init__(in_channels, label_type)
        ...
        if batchnorm is True:
            for i in range(num_repeats):
                model.append(
                    nn.Sequential(
                        SpectralNorm2d(nn.Conv2d(d, d * 2, 4, 2, 1, bias=use_bias)),
                        nn.BatchNorm2d(d * 2),
                        nl,
                    )
                )
            ...
            for i in range(num_repeats):
                model.append(
                    nn.Sequential(SpectralNorm2d(nn.Conv2d(d, d * 2, 4, 2, 1, bias=use_bias)), nl)
                )
                d *= 2
        self.disc = nn.Sequential(SpectralNorm2d(nn.Conv2d(d, 1, 4, 1, 0, bias=use_bias)), last_nl)
        self.model = nn.Sequential(*model)
        self._weight_initializer()
...
#         super(SNGANGenerator, self).__init__(encoding_dims, "none")
...
#             SpectralNorm2d(nn.ConvTranspose2d(d // 8, out_channels, 4, 2, 1, bias=True)),
#             nn.Tanh(),
#         )
...
#         super(SNGANDiscriminator, self).__init__(in_channels, "none")
#         d = step_channels
#         self.model = nn.Sequential(
