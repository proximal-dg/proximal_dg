...
class Logger(object):
    ...
    def __init__(
        ...
        self.logger_end_epoch.append(
            ImageVisualize(
                trainer, writer=self.writer, test_noise=test_noise, nrow=nrow,verbose=self.verbose
            )
        )
        self.logger_mid_epoch.append(
            GradientVisualize(trainer.model_names, writer=self.writer,verbose=self.verbose)
        )
        if metrics_list is not None:
            self.logger_end_epoch.append(
                MetricVisualize(metrics_list, writer=self.writer,verbose=self.verbose)
            )
        self.logger_mid_epoch.append(LossVisualize(losses_list, writer=self.writer,verbose=self.verbose))
